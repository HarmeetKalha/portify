...
export default function ProfileView() {
  ...
  const { addToRecentlyViewed, user, userType } = useAuth();
  ...
  const [applyForm, setApplyForm] = useState(() => ({
    ...
    location: '',
    resumeUrl: '',
    portfolioUrl: user?.portfolio?.socials?.portfolio || '',
    linkedinUrl: user?.portfolio?.socials?.linkedin || '',
    githubUrl: user?.portfolio?.socials?.github || '',
    coverLetter: ''
  }));
  ...
  // Add to recently viewed (avoid state updates during render)
  useEffect(() => {
  ...
  if (isEmployer) {
    const canApply = userType === 'employee' && !!user && Number(user.id) !== Number(profile.id);
    const alreadyApplied = canApply ? hasEmployeeApplied(profile.id, user.id) : false;
    ...
    const openApply = () => {
      ...
      setApplyForm(prev => ({
        ...
        portfolioUrl: user?.portfolio?.socials?.portfolio || prev.portfolioUrl,
        linkedinUrl: user?.portfolio?.socials?.linkedin || prev.linkedinUrl,
        githubUrl: user?.portfolio?.socials?.github || prev.githubUrl
      }));
      setIsApplyOpen(true);
    };
    ...
    const submitApplication = (e) => {
      ...
      if (!applyForm.fullName.trim() || !applyForm.email.trim() || !applyForm.phone.trim()) {
        setApplyError('Please fill in your full name, email, and phone.');
        return;
      }
      if (alreadyApplied) {
        setApplyError('You already applied to this company.');
        return;
      }
      ...
      addApplication({
        id: Date.now(),
        createdAt: Date.now(),
        employerId: Number(profile.id),
        employerName: profile.companyName || profile.name,
                ...
                {canApply && (
                  <div style={{ marginTop: 12, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    <button
                      ...
                      title={alreadyApplied ? 'You already applied to this company' : 'Apply to this company'}
                    >
          ...
          <div className="modal-overlay" role="dialog" aria-modal="true">
            <div className="modal-card glass-card">
              <div className="modal-header">
                <h2>Apply to {profile.companyName || profile.name}</h2>
                <button className="modal-close" onClick={() => setIsApplyOpen(false)} aria-label="Close">
                  ×
                </button>
              </div>
              ...
              <form onSubmit={submitApplication} className="modal-form">
                ...
                <div className="modal-actions">
                  <button type="button" className="btn btn-outline" onClick={() => setIsApplyOpen(false)}>
                    Cancel
                  </button>
